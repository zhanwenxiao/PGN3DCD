...
class CDTracker(BaseTracker):
    ...
    def track(self, model: model_interface.TrackerInterface, conv_classes=None, **kwargs):
        ...
        if type(outputs) == list:
            self._compute_metrics_list(outputs, targets, conv_classes)
        else:
            self._compute_metrics(outputs, targets, conv_classes)
        return outputs, targets

    def _compute_metrics_list(self, outputs, labels, conv_classes=None):
        ...
        self._confusion_matrix.count_predicted_batch_list(labels0, pred0, labels1, pred1)
