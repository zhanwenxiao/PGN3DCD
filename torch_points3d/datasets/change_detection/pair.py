    ...
    def to_data_list(self, target=1):
        ...
        for p in range(batch_size):
            ...
            for key, item in self:
                ...
                # if torch.is_tensor(item) and key not in ['batch', 'batch_target', 'y', 'y0','y1','pred','pred1',
                #                                          'ptr','ptr_target', 'gt_change']:
                    if 'target' in key:
            ...
            if hasattr(self, 'y_target'):
                pair.y_target = self.y_target[idx_target_p]
            ...
            if hasattr(self, 'pred') and self.pred is not None:
                # pair.pred0 = self.pred[0][idx_p]
                # pair.pred1 = self.pred[1][idx_target_p]
                if target == 1:
